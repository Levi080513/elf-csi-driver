...
- name: csi-snapshotter
  ...
  resources: {{ toYaml .Values.sidecar.resources.csi_snapshotter | nindent 4 }}
  args:
  - "--csi-address=$(ADDRESS)"
...
- name: csi-resizer
  ...
  resources: {{ toYaml .Values.sidecar.resources.csi_resizer | nindent 4 }}
  args:
    - "--csi-address=$(ADDRESS)"
    - "--leader-election"
    - "--v=5"
...
- name: driver-registrar
  securityContext:
    privileged: true
  image: {{ include "csi-driver.sidecar.registry" . }}/csi-node-driver-registrar:v2.0.0
  resources: {{ toYaml .Values.sidecar.resources.driver_registrar | nindent 4 }}
  args:
    - --v=5
    - --csi-address=/csi/csi.sock
